...
/**
 * 게시판 모듈 메일 발송 이력 개발용 시더
 *
 * 게시판 메일 템플릿(new_comment, reply_comment, post_reply, post_action, new_post_admin)을
 * 기반으로 발송 이력 샘플 데이터를 생성합니다.
 *
 * 개발/테스트 용도이므로 설치 시 자동 실행되지 않습니다.
 * 수동 실행: php artisan module:seed sirsoft-board --class=BoardMailSendLogSeeder
 */
class BoardMailSendLogSeeder extends Seeder
{
    use HasSeederCounts;

    /**
     * 게시판 메일 템플릿 유형별 제목 패턴
     *
     * @var array<string, string>
     */
    private array $templateSubjects = [
        'new_comment' => '[그누보드7] 게시글에 새 댓글이 등록되었습니다',
        'reply_comment' => '[그누보드7] 댓글에 답글이 등록되었습니다',
        'post_reply' => '[그누보드7] 게시글에 답변글이 등록되었습니다',
        'post_action' => '[그누보드7] 게시글 관리자 처리 안내',
        'new_post_admin' => '[그누보드7] 새 게시글이 등록되었습니다',
    ];
    ...
    {
        $count = $this->getSeederCount('mail_send_logs', 25);

        $this->command->info('게시판 메일 발송 이력 시딩 시작...');
        ...
        $this->command->info("게시판 메일 발송 이력 시딩 완료 ({$count}건)");
    }
