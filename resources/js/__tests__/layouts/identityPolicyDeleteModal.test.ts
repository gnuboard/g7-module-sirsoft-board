/**
 * 게시판 환경설정 > 본인인증 정책 탭 — 삭제 핸들러 회귀 테스트.
 *
 * 회귀 사례 (#297):
...
const tabPartial = require('../../../layouts/admin/partials/admin_board_settings/_tab_identity_policies.json');
const deleteModal = require('../../../layouts/admin/partials/admin_board_settings/_modal_identity_policy_delete.json');
...
describe('게시판 본인인증 정책 삭제 — confirm 핸들러 미사용, 모달 + 스피너 패턴 (#297)', () => {
    it('탭 partial 에 confirm 핸들러가 존재하지 않는다', () => {
        expect(JSON.stringify(tabPartial)).not.toMatch(/"handler"\s*:\s*"confirm"/);
    });

    it('삭제 버튼은 setState boardIdentityPolicyDeleteId + openModal board_identity_policy_delete_modal 시퀀스 사용 (PC + 모바일)', () => {
        ...
        const deleteSequences = sequences.filter((seq) => {
            ...
            const hasSetDeleteId = actions.some(
                (a: any) =>
                    a.handler === 'setState' && a.params?.boardIdentityPolicyDeleteId !== undefined,
            );
            const hasOpenDeleteModal = actions.some(
                (a: any) =>
                    a.handler === 'openModal' && a.target === 'board_identity_policy_delete_modal',
            );
    ...
    it('삭제 모달이 부모 레이아웃의 modals 배열에 등록되어 있다', () => {
        ...
        expect(partials).toContain('partials/admin_board_settings/_modal_identity_policy_delete.json');
    });

    it('삭제 모달은 boardIdentityPolicyDeleteId 를 사용한 DELETE API 호출 + boardIdentityPolicies 데이터소스 refetch 를 수행', () => {
        const apiCalls = collectNodes(deleteModal, (n) => n.handler === 'apiCall');
        const matching = apiCalls.find(
            (c) =>
                typeof c.target === 'string' &&
                c.target.includes('{{_global.boardIdentityPolicyDeleteId}}') &&
                c.params?.method === 'DELETE',
        );
        expect(matching).toBeTruthy();

        const refetches = collectNodes(matching, (n) => n.handler === 'refetchDataSource');
        expect(refetches.some((r) => r.params?.dataSourceId === 'boardIdentityPolicies')).toBe(true);
    });

    it('삭제 모달은 _global.boardIdentityPolicyDeleting 기준으로 disabled + 스피너 노출', () => {
        const stringified = JSON.stringify(deleteModal);
        expect(stringified).toMatch(/_global\.boardIdentityPolicyDeleting/);
